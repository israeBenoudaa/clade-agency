...
import { exportProgrammeExcel } from '../../utils/exportTableExcel'
...
function defaultItem() {
  return { id: `i${Date.now() + Math.random()}`, programme: '', unite: 'u', quantite: 1, superficieUnitaire: 0 }
}
...
export default function ProgrammePage() {
  ...
  const { projects, updateProgramme } = useData()
  ...
  useEffect(() => {
    ...
    setStatut(prog.statut || 'en_cours')
    setGroupes(
      prog.groupes?.length
        ? prog.groupes
        : [DEFAULT_GROUPE()]
  ...
  const save = useCallback((newGroupes, newStatut) => {
    if (!project) return
    updateProgramme(project.id, { statut: newStatut, groupes: newGroupes })
  }, [project, updateProgramme])
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h1 className="font-display text-2xl text-ink">Programmation</h1>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => exportProgrammeExcel({ statut, groupes }, project.nom)}
            className="btn-ghost text-sm"
          >
            <Download size={15} />
            Excel
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr style={{ background: '#0A1E3F' }}>
                {['Catégorie', 'Programme', 'Unité', 'Quantité', 'Sup. unit. (m²)', 'Sup. totale (m²)', ''].map(h => (
                  <th
                    key={h}
                    className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider"
                    style={{ color: h ? 'rgba(255,255,255,0.85)' : 'transparent', whiteSpace: 'nowrap' }}
                  >
                    {h || '·'}
                  </th>
                ))}
            ...
            <tbody>
              {groupes.flatMap(group => {
                ...
                const subTotal = items.reduce((s, i) => s + calcSt(i), 0)
                ...
                return [
                    ...
                    >
                      {idx === 0 && (
                        <td
                          ...
                          style={{ background: '#C5D3E8', verticalAlign: 'middle', width: 140 }}
                        ...
                        >
                          <div className="flex items-start gap-1">
                            ...
                            <CellInput
                              value={group.nom}
                              onChange={v => updateGroupeNom(group.id, v)}
                              placeholder="Catégorie"
                              className="font-semibold text-[#0A1E3F] text-[11px] uppercase tracking-wide"
                            />
                          </div>
                          <button
                          ...
                          >
                            <Trash2 size={10} /> Supprimer groupe
                          </button>
                        </td>
                      )}
                      <td className="px-3 py-2 border-r border-border/30" style={{ minWidth: 180 }}>
                        <CellInput
                          value={item.programme}
                          onChange={v => updateItem(group.id, item.id, 'programme', v)}
                      ...
                      <td className="px-3 py-2 border-r border-border/30" style={{ width: 80 }}>
                        <CellInput
                          value={item.unite}
                          onChange={v => updateItem(group.id, item.id, 'unite', v)}
                          placeholder="u"
                          className="text-center"
                        />
                      </td>
                      <td className="px-3 py-2 border-r border-border/30" style={{ width: 90 }}>
                        <CellInput
                          type="number"
                          value={item.quantite}
                          onChange={v => updateItem(group.id, item.id, 'quantite', v)}
                          className="text-right"
                        />
                      </td>
                      <td className="px-3 py-2 border-r border-border/30" style={{ width: 130 }}>
                        <CellInput
                          type="number"
                          value={item.superficieUnitaire}
                          onChange={v => updateItem(group.id, item.id, 'superficieUnitaire', v)}
                          className="text-right"
                        />
                      </td>
                      <td className="px-3 py-2 border-r border-border/30 text-right font-semibold text-ink" style={{ width: 130 }}>
                        {calcSt(item) || '—'}
                      </td>
                  ...
                  <tr key={`st_${group.id}`} style={{ background: '#F3F4F6' }} className="border-b border-border">
                    <td
                      colSpan={5}
                      className="px-4 py-2 text-xs font-bold text-ink/70 uppercase tracking-wide"
                    >
                      Sous-total — {group.nom}
                    </td>
                    <td className="px-4 py-2 text-right font-bold text-ink">
                      {subTotal} <span className="text-xs font-normal text-muted">m²</span>
                    </td>
                    <td />
                  </tr>,
                ]
              ...
              <tr style={{ background: '#0A1E3F' }}>
                <td
                  colSpan={5}
                  className="px-4 py-3 font-bold text-xs uppercase tracking-widest"
                  style={{ color: 'rgba(255,255,255,0.85)' }}
                >
                  SUPERFICIE TOTALE GLOBALE
                </td>
                <td className="px-4 py-3 text-right font-bold text-white text-base">
                  {grandTotal} <span className="text-xs font-normal opacity-70">m²</span>
                </td>
